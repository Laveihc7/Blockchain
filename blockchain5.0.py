...
def next_block(last_block):
	...
	#prev_proof = get_info_by_index(index, 'proof')
...
def save_block(block_to_add):
	...
	with open(filename, 'w') as file:
		...
		#file.write('\tdiary=' + str(block.message) + '\n')
		#file.write('\tproof=' + str(block.proof) + '\n')
		file.write('}\n')
...
def generate_new_block():
	...
	#proof = proof_of_work(get_info_by_index(index, 'proof'))
	data = 'Block #' + str(index)
	#msg = upload_msg
...
def get_hash_by_index(index):
	...
	return key_value_pair[-1].rstrip()

def upload_msg(block_num, msg):
	filename = None
	for file in os.listdir('blocks/'):
		if str(block_num) in file:
			filename = str(file)
			break

	with open('blocks/Block#{}.txt'.format(block_num), 'a') as target:
		target.write('\nYour diary begins:\n' + msg)

def continue_diary(block_num, msg):
	filename = None
	for file in os.listdir('blocks/'):
		if str(block_num) in file:
			filename = str(file)
...
if __name__ == '__main__':
	...
	while True:
		...
		print('4. Write diary in a Block')
		print('5. Continue diary in a Block')
		...
		try:
			...
			if service == 1:
				...
				diary = str()
				block_num = input('Choose the block you wanna diary: ')
				if block_num <= str(index):
					diary = input('\nPlease go: ')
					upload_msg(block_num, diary)
				else:
					print('\nError: That is not a block number')
			elif service == 5:
				diary = str()
				block_num = input('Choose the block you wanna continue diary: ')
				if block_num <= str(index):
					diary = input('\nPlease continue: ')
					continue_diary(block_num, diary)
				else:
					print('\nError: That is not a block number')
			else:
				print('\nError.')
